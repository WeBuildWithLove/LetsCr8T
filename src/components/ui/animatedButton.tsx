...
const AnimatedButton: React.FC<AnimatedButtonProps> = ({
  ...
  const baseClasses = `
    ...
    before:bg-cr8tOrange before:dark:bg-cr8tBlack font-manrope font-semibold text-base shadow-[15%]
  `;
  ...
  return (
    <button onClick={onClick} className={clsx(baseClasses, className)}>
      {children}
      {showSettingsIcon && (
        <SettingsIcon className="settings-icon group-hover:animate-spin" />
      )}
      {withHeartbeat && (
        <div
