...
const FAQs: React.FC = () => {
  // Memoize FAQs data to avoid unnecessary re-renders
  const faqData = useMemo(() => FAQS, []);

  return (
    <section className="lg:px-[80px] md:pt-[150px] pt-[50px] px-6 relative">
      <div className="flex lg:justify-between flex-wrap lg:flex-nowrap gap-[25px] relative justify-center">
        {/* Left Section */}
        <div className="flex flex-col lg:gap-[25px] gap-5 relative z-10">
          <h1 className="text-cr8tOrange font-urban md:text-2xl font-normal text-base">
            FAQs
          </h1>
          <p className="font-semibold md:text-5xl lg:text-3xl text-2xl leading-9 md:leading-[62px] text-Cr8tGray pt-2 md:pt-4 lg:w-[350px] xl:w-[571px]">
            What would you like to know about LetsCr8T?
          </p>
        ...
        <Accordion
          type="single"
          collapsible
          className="lg:h-[500px] h-[530px] md:w-[641px] w-full"
        >
          {faqData.map((item) => (
            <AccordionItem
              value={item.id}
              key={item.id}
              className="rounded-[15px] px-4 md:px-[30px] py-3 font-poppins bg-[#F8F8F8] md:w-full lg:w-[450px] xl:w-[641px] w-full overflow-hidden mb-5"
            >
              <AccordionTrigger className="h-[60px] text-left md:text-lg text-base flex items-center transition-all duration-300">
                {item.question}
              </AccordionTrigger>
              <AccordionContent className="text-cr8tLightBlack md:leading-7 leading-6 md:text-lg text-base transition-all duration-300">
                {item.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
        ...
        {/* Decorative Question Mark for Mobile */}
        <Image
          src={QuestionMarkIcon}
          alt="decorative question mark icon"
          width={342}
          height={343}
          className="lg:hidden block opacity-30 absolute top-[300px] md:top-[400px] left-1/2 transform -translate-x-1/2 pointer-events-none z-0"
        />
